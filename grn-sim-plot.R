...
theme_update(coord_cartesian(xlim=c(0,30),ylim=c(0,30)))
...
g.0.1 = ggplot(df.0, aes(x=ip1,y=ip2,xend=p1,yend=p2,color=p1)) + geom_segment() + theme(legend.position="none") +
  facet_grid(~euler) + coord_cartesian(xlim = c(0,30), ylim = c(0,30))
...
ggarrange(g.0.1, g.0.2, widths = c(2,1), labels = c("A", "B"))
...
ggarrange(g.1.1, g.1.2, g.1.3, g.1.4, nrow=2, ncol=2, labels=c("A", "B", "C", "D"))
...
ggarrange(g.3, g.4, labels=c("A", "B"))
...
g.5 = ggplot(df.5, aes(x=ip1,y=ip2,xend=p1,yend=p2,color=p1)) + geom_segment() +
  theme(legend.position="none") + facet_grid(ATP ~ ICs) + coord_cartesian(xlim = c(0,30), ylim = c(0,30))
...
ggarrange(g.6a, g.6b, g.6c, labels=c("A", "B", "C"))
...
ggarrange(g.7.1, g.7.2, labels=c("A", "B"))
...
g.8.1.1 = ggplot(df.8[df.8$scale==0,], aes(x=ip1,y=ip2,xend=p1,yend=p2,color=p1)) + geom_segment() +
  facet_grid(param.labels[param1+1] ~ param.labels[param2+1]) + coord_cartesian(xlim = c(0,30), ylim = c(0,30))
...
ggarrange(g.8.1.1, g.8.1.2, g.8.1.3, g.8.1.4, labels=c("A", "B", "C", "D"))

ggarrange(g.8.2.1, g.8.2.2, g.8.2.3, g.8.2.4, labels=c("A", "B", "C", "D"))
