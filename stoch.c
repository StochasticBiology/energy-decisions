...
int main(void)
{
  ...
  // loop through different parameterisations
  // we'll only do one instance of stochastic simulation each for now -- obviously this means we can't get variance statistics
  for(expt = 0; expt <= 3; expt++)
    {
      sprintf(fstr, "gillespie-series-%i.csv", expt);
      fp = fopen(fstr, "w");
      fprintf(fp, "t, pro_1, pro_2, prooff_1, prooff_2, rna_1, rna_2, p_1, p_2, pp_1, pp_2\n");

      sprintf(fstr, "gillespie-switches-%i.csv", expt);
      fp1 = fopen(fstr, "w");
      fprintf(fp1, "i, dt\n");
      ...
      // make experiment-specific parameter changes
      if(expt >= 1) cd2 = 0.04;
      if(expt == 2) { lambda2 = 0.0067/4; lambda3 = 0.0474/4; }
